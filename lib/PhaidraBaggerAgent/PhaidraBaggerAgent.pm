...
sub _init_pafdb {
        ...
        my $mongoPaf = MongoDB::MongoClient->new(
                                              host => $self->{config}->{instances}->{$ingest_instance}->{paf_mongodb}->{host}, 
                                              username => $self->{config}->{instances}->{$ingest_instance}->{paf_mongodb}->{username}, 
                                              password => $self->{config}->{instances}->{$ingest_instance}->{paf_mongodb}->{password}, 
                                              db_name => $self->{config}->{instances}->{$ingest_instance}->{paf_mongodb}->{database}
                                             );
        
        my $mongoPafDb = $mongoPaf->get_database($self->{config}->{instances}->{$ingest_instance}->{paf_mongodb}->{database});
        $self->{pafdb} = $mongoPafDb;
...
sub run_job {
        ...
        while (my $bag = $bags->next) {
                ...
                my $asset;
                my $getOctetsAlertsPush;
                
                if($type eq 'push'){
                     #new
                     # ingest bag
                     my $getOctetsRes = $self->_getOctets($bag->{old_pid}, $bag->{origin_instance});
                     if(@{$getOctetsRes}[0]){
                           $asset = @{$getOctetsRes}[0];
                     }else{
                           $getOctetsAlertsPush = @{$getOctetsRes}[1];
                     }
                }elsif($type eq 'metadata_update'){
                }else{
                ...
                if($type eq 'metadata_update'){ 
                        ...
                        #new
                        # error occurred while accessing octets from original instance (push)
                        if($getOctetsAlertsPush){
                                $self->{'log'}->info(Dumper($getOctetsAlertsPush));
                                $self->{bags_coll}->update({bagid => $bag->{bagid}, 'jobs.jobid' => $jobid, project => $job->{project}},{'$set' => {'jobs.$.alerts' => $getOctetsAlertsPush}});
                                next;
                        }
                        
                        # ingest bag
                        #my ($pid, $alerts) = $self->_ingest_bag($filepath, $bag, $ingest_instance, $username, $password);
                        my ($pid, $alerts) = $self->_ingest_bag($asset, $bag, $ingest_instance, $username, $password);
                        $self->{'log'}->info("Ingested bagid[".$bag->{bagid}."] pid[$pid]") if(defined($pid));
                        $self->{'log'}->info("Ingested bagid[".$bag->{bagid}."] alerts[".Dumper($alerts)."]") if(defined($alerts));
                        # update alerts
                        if(defined($alerts)){
                                if(scalar @{$alerts} > 0){
                                        $self->{'log'}->info(Dumper(\@alerts));
                        ...
                        if($pid){
                                ...
                                #add new_pid to jobs collection
                                $self->{'log'}->info('bag _id:'.$bag->{_id});
                                $self->{jobs_coll}->update({'_id' => MongoDB::OID->new(value => $jobid), 'bags.bag_id' => $bag->{_id}->{value}},
                                  {'$set' => {'bags.$.new_pid' => $pid}});
                        }else{
                              $self->{'log'}->info("Error: new pid is not created.");
                              print "Error: new pid is not created.\n"
                        }
                        # insert event
                ...
                #mark bag as 'done'
                $self->{bags_coll}->update({bagid => $bag->{bagid}, project => $job->{project}},{'$set' => {'status' => 'done'}});
                
                $self->{'log'}->info("[$i/$count] Done ".$bag->{bagid});
        }
        
        #$self->_send_notification_to_owner($job) if $job->{notify_user_on_success} eq 1;
...
sub _ingest_bag {
        ...
        $self->{'log'}->info("Ingest bag=".$bag->{bagid}.", to instance=$ingest_instance");
        ...
        #my $tx = $self->{ua}->post($url  => form => { file => { file => $filepath }, metadata => $json });
        my $tx = $self->{ua}->post($url  => form => { file => { file => $asset }, metadata => $json });
...
sub _ingest_bag_old {
        ...
        # $bag->{cmodel}
        unless(defined $bag->{cmodel}){
               push(@alerts, { type => 'danger', msg => "Cmodel is not defined." });
               return ($pid, \@alerts);
        }
        
        
        if($bag->{cmodel} eq 'cmodel:PDFDocument' ){
               if(exists($base[1])){
                       $url->path($base[1]."/document/create");
               }else{
                       $url->path("/document/create");
               }
        }elsif($bag->{cmodel} eq 'cmodel:Picture'){
               if(exists($base[1])){
                       $url->path($base[1]."/picture/create");
               }else{
                       $url->path("/picture/create");
               }
        }elsif($bag->{cmodel} eq 'cmodel:Video'){
               if(exists($base[1])){
                       $url->path($base[1]."/video/create");
               }else{
                       $url->path("/video/create");
               }
        }elsif($bag->{cmodel} eq 'cmodel:Audio'){
               if(exists($base[1])){
               ...
               push(@alerts, { type => 'danger', msg => "Unknown cmodel." });
               return ($pid, \@alerts);
        }
...
sub _getOctets{
     ...
     my $self = shift; 
     my $old_pid = shift;
     my $origin_instance = shift;
     
     
     my @alerts = ();
     my $result;
     
     my $instanceData = $self->_getInstanceData($origin_instance);
     my @base = split('/',$instanceData->{apibaseurl});
     my $url = Mojo::URL->new;
     $url->scheme($instanceData->{scheme});
     $url->host($base[0]);
     $url->userinfo($instanceData->{adminusername}.":".$instanceData->{adminpassword});
     if(exists($base[1])){
     ...
     if (my $res = $tx->success) {
         my $res_data =  $tx->res->json;
                ...
                if($tx->res->json){
                        if($tx->res->json->{alerts}){
                                $self->{'log'}->info("getOctetsRes error:", Dumper($tx->res->json->{alerts}));
                                my @res;
                                push @res, undef;
                                push @res, $tx->res->json->{alerts};
                                return \@res;
                        }
                }
                ...
                if (ref($err) eq "REF"){
                      if ($err->{code}){
                            push(@alerts, { type => 'danger', ts => time, step => '/object/old_pid/octets', msg => $err->{code}." response: ".$err->{message} });
                      }else{
                            push(@alerts, { type => 'danger', ts => time, step => '/object/old_pid/octets', msg => "Connection error: ".$err->{message} });
                      }
                }else{
                      push(@alerts, { type => 'danger', ts => time, step => '/object/old_pid/octets', msg => $err });
                }
                $self->{'log'}->error(Dumper(\@alerts));
                $self->{'log'}->info("getOctetsRes error:", Dumper(\@alerts));
                my @res;
                push @res, undef;
                push @res, \@alerts;
                return \@res;
...
sub _getInstanceData {
   
       my $self = shift; 
       my $origin_instance = shift;
       
       foreach my $key ( keys %{$self->{config}->{instances}} ){
              if($self->{config}->{instances}->{$key}->{id} eq $origin_instance){
                   return $self->{config}->{instances}->{$key};
              }
       }
       return;
}

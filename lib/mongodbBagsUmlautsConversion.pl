...
=head1
    ...
    perl mongodbBagsUmlautsConversion.pl 5671244606b2af0d90020000  /your/path/to/PhaidraBagger.json
...
my $collection = $database->get_collection( 'bags' );
...
while (my $doc = $dataset->next) {
        ...
        my $fixedFile = decode("utf-8-de", $doc->{file});
        utf8::upgrade($fixedFile);

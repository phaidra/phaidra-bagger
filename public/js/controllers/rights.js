app.controller('RightsCtrl', function($scope, $rootScope, $modal, $location, $timeout, MetadataService, FrontendService, DirectoryService, promiseTracker) {
   ...
   $scope.getRightsObject = function() {
        ...
        var promise = MetadataService.getRightsObject($scope.initdata.bagid);
    	$scope.loadingTracker.addPromise(promise);
    	promise.then(
        	function(response) {
        		$scope.alerts = response.data.alerts;
			if(typeof response.data.metadata.rights !== 'undefined'){
	...
	for (i = 0; i < $scope.faculties_array.length; ++i) {
	    if(typeof $scope.faculties_array[i].dt_end === 'undefined'){
			...
			if(typeof $scope.faculties_array[i].dt_end !== 'undefined'){
			      if($scope.faculties_array[i].dt_end != ''){
        ...
        var promise = MetadataService.saveRightsObject($scope.initdata.bagid, rights);
    	$scope.loadingTracker.addPromise(promise);
    	promise.then(
        	function(response) {
        		$scope.alerts = response.data.alerts;
    ...
    $scope.addNewUser = function(date, newU) {
      	  
              if(newU){
                       var newUser = {};
                       newUser.id = newU.uid;
                       newUser.value = newU.value;
	               newUser.dt_end = date;
